...
void Hal::init()
{
    ...
    *mHalCout << "ok" << endl;
    ...
    // V2: register button pins with fixed names matching BTN_* defines
    static const char* buttonNames[] = {
        "start-pause", "goto-zero", "rewind", "probe-z",
    ...
    {
        mButtonNameToIdx[std::string(mComponentPrefix) + ".button." + buttonNames[idx]] = idx;
...
{
    setPin(enabled, "macro-1");
}
// ----------------------------------------------------------------------
void Hal::setMacro2(bool enabled)
{
    setPin(enabled, "macro-2");
}
// ----------------------------------------------------------------------
...
{
    setPin(enabled, "macro-3");              // Hardcoded toggle spindle direction
}
// ----------------------------------------------------------------------
...
{
    setPin(enabled, "macro-1");                        // Hardcoded Absolute/relative Dro
}
// ----------------------------------------------------------------------
...
void Hal::setPin(bool enabled, const char* pinName)
{
    ...
    auto it = mButtonNameToIdx.find(fullyQualifiedPinName);
    if (it == mButtonNameToIdx.end())
    {
        *mHalCout << "hal   unknown button pin: " << pinName << endl;
        return;
    }
    setPin(enabled, it->second, pinName);
}
// ----------------------------------------------------------------------
