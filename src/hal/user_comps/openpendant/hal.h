...
#ifndef OPENPENDANT_HAL_H
#define OPENPENDANT_HAL_H
...
// Consider renaming HalMemory to HalPins or similar if it only holds pins
class HalMemory // Or rename to HalPins
{
public:
    // --- Input Pins (Read by Component from HAL) ---
    // Adapt these based on the feedback needed for your pendant
    struct In
    {
    public:
    ...
    public:
        ...
        Out(); // Keep constructor if needed
    };

    In  in;
    Out out;

    HalMemory(); // Or HalPins()
    ~HalMemory(); // Or ~HalPins()
};
// ----------------------------------------------------------------------
//! HAL and related parameters
// Consider renaming Hal to HalManager or similar
class Hal // Or rename to HalManager
{
public:
    // Remove HalRequestProfile if Profiles namespace is removed
    Hal(Profiles::HalRequestProfile halRequestProfile=Profiles::halRequestSlowProfile()); // <<< REVIEW / REMOVE / ADAPT
    ~Hal();
