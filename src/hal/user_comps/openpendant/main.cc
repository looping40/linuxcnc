...
OpenPendant::OpenPendantComponent* OpenPdtComponent{nullptr};
// ----------------------------------------------------------------------
...
{
    if (OpenPdtComponent != nullptr)
    {
        OpenPdtComponent->requestTermination(signal);
    }
}
// ----------------------------------------------------------------------
...
{
    OpenPdtComponent = new OpenPendant::OpenPendantComponent();

    const char* optargs = "phaeHutnU:v:";
    for (int opt = getopt(argc, argv, optargs); opt != -1; opt = getopt(argc, argv, optargs))
    {
        switch (opt)
        {
            case 'H':
                OpenPdtComponent->setSimulationMode(false);
                break;
            case 't':
                OpenPdtComponent->setWaitWithTimeout(3);
                break;
            case 'e':
                OpenPdtComponent->enableVerbosePendant(true);
                break;
            case 'u':
                OpenPdtComponent->enableVerboseInit(true);
                OpenPdtComponent->enableVerboseRx(true);
                break;
            case 'U':
                OpenPdtComponent->enableVerboseInit(true);
                OpenPdtComponent->enableVerboseRx(true);
                OpenPdtComponent->enableVerboseTx(true);
                break;
            case 'p':
                OpenPdtComponent->enableVerboseInit(true);
                OpenPdtComponent->enableVerboseHal(true);
                break;
            case 'a':
                OpenPdtComponent->enableVerboseInit(true);
                OpenPdtComponent->enableVerbosePendant(true);
            ...
            case 'n':
                break;
            case 'h':
                return printUsage(basename(argv[0]), OpenPdtComponent->getName());
                break;
            default:
                return printUsage(basename(argv[0]), OpenPdtComponent->getName(), true);
                break;
        }
    }
    ...
    OpenPdtComponent->run();

    //! hotfix for https://github.com/machinekit/machinekit/issues/1266
    if (OpenPdtComponent->isSimulationModeEnabled())
    {
//        google::protobuf::ShutdownProtobufLibrary();
    }

    delete (OpenPdtComponent);
    return 0;
}
